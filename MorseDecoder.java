...
public class MorseDecoder {
	...
	public static char Decoder(String code) {
		...
		while (morseCode[i].equals("Unknown Code") == false) {
			if (code.equals(morseCode[i])) // Once a match is found, the method will return the character corresponding
											// to that code.
			{
	...
	public static void main(String[] args) throws Exception {
		...
		greenButton.addStateChangeListener(new DigitalInputStateChangeListener() {
			public void onStateChange(DigitalInputStateChangeEvent e) {
				...
				if (e.getState() == false) {
					timePressed = ((double) (System.currentTimeMillis() - startPress) / 1000); // Grabs time when the
																								// button is released,
																								// and subtracts start
																								// time. Converts from
																								// ms to seconds.
